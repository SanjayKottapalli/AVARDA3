...
#		unique_xri = xri_index - shared_all; unique_xrj = xrj_index - shared_all #unique xr
		...
		d1[(i,j)] = len(filter_unique_j)/(len(peptide_lib)-len(i_shared)-(len(unique_j)-len(filter_unique_j)))
		# above, but for virus i
		d1[(j,i)] = len(filter_unique_i)/(len(peptide_lib)-len(j_shared)-(len(unique_i)-len(filter_unique_i)))
...
if __name__ == "__main__": # main method
	...
	second_round_prob.to_csv("unique_probabilities_20181020.csv", header=True, index=True) #print to file
