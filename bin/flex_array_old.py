	...
	while len(ranked_hits)>0 and ranked_hits.iloc[0]>0:
		...
		if specie[0] == first_sp:
			for i in specie[1:]:
				...
				virus_evi = {i:virus_evi[i][virus_evi[i]>=80] for i in zb_df.columns}
				########
				high_index = set(virus_evi[specie[0]][specie[0]].dropna().index); i_index = set(virus_evi[i][specie[0]].dropna().index)
				xrhigh_index = set(virus_xr[specie[0]][specie[0]].dropna().index); xri_index = set(virus_xr[i][specie[0]].dropna().index)
		...
		filter_top = virus_evi[top_hit][top_hit].dropna()#zb_df.loc[zb_df[top_hit], top_hit]
		filter_top = gen_ind_hits(n_hits.loc[filter_top.index], dep_pep)
		
		n_rank = len(gen_ind_hits(n_hits.loc[list(set(n_hits.index)-(set(virus_evi[top_hit][top_hit].dropna().index)-set(filter_top.index)))], dep_pep))
		...
		ranked_hits = {i:len(virus_tot_filter[i]) for i in specie}
		ranked_hits = pd.Series(ranked_hits)
		#ranked_hits = pd.Series(index=list(zb_df.columns), data=[len(i) for i in virus_tot_filter])
