...
def calc_pair(pair, virus_aln, virus_xr, dep_pep, first_round_prob, peptide_lib, iter_number, total_iter): #function to calculate unique probabilities
	...
	if len(shared_evidence) == 0:
		...
		d1[(i,j)] = len(filter_unique_j)/(len(peptide_lib)-len(shared_evidence)-len(unique_xrj)-(len(unique_j)-len(filter_unique_j)))
		# above, but for virus i
		d1[(j,i)] = len(filter_unique_i)/(len(peptide_lib)-len(shared_evidence)-len(unique_xri)-(len(unique_i)-len(filter_unique_i)))
...
if __name__ == "__main__": # main method
	...
	virus_tot_filter = [flex_array.gen_ind_hits(virus_aln[i], dep_pep) for i in virus_aln] # dictionary of filtered evidence peptides
	...
	first_round_prob.to_csv("total_probabilities_20181008.csv", header=False, index=True) #print to file
	print("First probability file generated.", flush=True)
	
	pair_time = timeit.default_timer()
	virus_pairs = list(combo(viruses[10:20], 2)) # list of all unique pairs of viruses under consideration (10 viruses for testing)
	...
	second_round_prob.to_csv("unique_probabilities_20181008.csv", header=True, index=True) #print to file
